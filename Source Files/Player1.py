...
class Asteroids(Animation):
    ...
    def keyPressed(self,event):
        if self.roundOver:
            if event.char == "r":
                self.contactServer("letterR.player1")
        if self.lobbyScreen == False:
            if event.keysym == "Up":
                self.contactServer("upKey.player1")
            elif event.keysym == "Left":
                self.contactServer("leftKeyPress.player1")
            elif event.keysym == "Right":
                self.contactServer("rightKeyPress.player1")
            elif event.keysym == "space":
                if self.instructionScreen and self.instructionPage == 2:
                    self.ibullets.append([self.shipX,self.shipY,self.shipDirection])
                else:     
                    self.contactServer("spaceKey.player1")

    def keyReleased(self,event):
        if self.roundOver == False:
            if event.keysym == "Left":
                self.contactServer("leftKeyRelease.player1")
            elif event.keysym == "Right":
                self.contactServer("rightKeyRelease.player1")
    ...
    def handleUserClose(self,event = None):
    ...
    def getUserName(self,event):
        ...
        if (((bx-bxr)<x<(bx+bxr)) and (
            ...
            if len(self.username) < 4:
                ...
                self.contactServer("enterUsername1." + self.username)
                self.toplevel.destroy()
    ...
    def mousePressed(self, event):
        ...
        if self.lobbyScreen:
            ...
            if self.clickedBox(event.x,event.y,mx,my,mxr,myr):
                self.creatingUserName = False
                self.contactServer("exitLobby.player1")
        ...
        if (((rx-rxr)<event.x<(rx+rxr)) and (
            ...
            if self.lobbyScreen and len(self.username2)>0:
                self.contactServer("readyToPlay.player1")
    ...
    def timerFired(self):
        ...
        try:
            ...
            self.startScreen = gameState["sS1"]
            self.lobbyScreen = gameState["lS1"]
    ...
    def drawLobbyScreen(self):
        if len(self.username2)>0:
            ...
            toptext = "Waiting for player 2 to join..."
            buttontext = "Waiting..."
            buttoncolor = "grey"
